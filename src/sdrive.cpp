...
/* TYMCZASOWA IMPLEMENTACJA
   TODO: Stworzyc prywatny obiekt sfolder i na nim wykonywac operacje
   TODO: Stworzyc dla sdrive osobny system wyjatkow */

sdrive::~sdrive(void) {
}

/* Kopiuje do folderu obiekty podane w parametrze */
void sdrive::store(std::vector< std::map< std::string, std::string > > src, 
...
void sdrive::extract(std::vector< std::map< std::string, std::string > > src, 
					  std::vector< std::map< std::string, std::string > > dest) {
	long len = 0, made = 0;
	dabFileInfo^ fInfo;
...
void sdrive::modify(std::vector< std::map< std::string, std::string > > src, 
					 std::vector< std::map< std::string, std::string > > dest) {
	long len = 0, made = 0;
	...
	/* Kopiowanie atrybutow plikow/folderow */
	for (unsigned int i = 0; i < src.size(); i++) {
		if (bCanceled) return;
			...
			try {
				...
				if ((dest[i][dabKeyName] != "") && (sPath1 != sPath2)) {
					if (!dabDir::Exists(dabToSysStr(sPath2.c_str()))) {
						...
						dabDir::Delete(dabToSysStr(sPath1.c_str()), true);
					}
				}
			}
			catch (...) {
...
void sdrive::copyInside(std::vector< std::map< std::string, std::string > > src, 
						 std::vector< std::map< std::string, std::string > > dest) {
	long len = 0, made = 0;
...
/* Zwraca pojemnosc dysku */
unsigned long long sdrive::getCapacity(void) {
	return 0;
}
