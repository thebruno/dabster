...
std::string str::_delimiter = "\\";
str::path str::_appPath;
...
str::path str::fixAppPath(void) {
	str::path appPath;
	try {
		std::string path = sysStrToCppStr(_commandLine::get());
...
std::string str::sysStrToCppStr(System::String^ sysStr) {
	char *str = (char*)(void*)(_marshal::StringToHGlobalAnsi(sysStr));
	std::string ret(str);
	_marshal::FreeHGlobal((System::IntPtr)str);
	return ret; 
}
